...
def detect_test(args):
    ...
    #model_path = os.path.join(model_dir, 'model', 'iface_freeze_model.pb')
    #print(model_path)
    ...
    iface = ia.iface_api()
    #iface = ia.iface_api(model_name)
    #iface = iface_api(saved_model_dir=model_dir)
    print('after init api', time.time())

    for i in range(1):
        print('before detec face', time.time())
        res = iface.detect_face(image_path)
        print('after detec face', time.time())
        print('detect result:', res)
        #print('before read img', time.time())
        print('before predi face', time.time())
        outputs = iface.predi_face_image(res['scaled'])
        print('after predi face', time.time())
        print('predict result:', outputs)
        #outputs=iface.detect_predi_face(image_path)
        img=misc.imread(image_path)
        print('before detect and predi face', time.time())
        outputs=iface.detect_predi_face_image(img)
        print('after detect and predi face', time.time())
        if outputs is None:
            return
        ...
        vis_util.save_image_array_as_png(img, os.path.join('/raid/download/tmp3', base+'_detected'+ext))
